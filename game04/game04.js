...
function setup() {
  ...
  pipasImg = [pipa01img, pipa02img, pipa03img, pipa05img, pipa06img, pipa07img, pipa08img, pipa09img]
}
...
function createWildPipa(){
  ...
  wildPipa = createSprite(position[0],position[1])

  wildPipaImg = loadImage(pipasImg[Math.floor(Math.random() * pipasImg.length)].elt.src)
  console.log(wildPipaImg)

  wildPipa.addImage(wildPipaImg)
  wildPipa.rotateToDirection = true;
  wildPipa.attractionPoint(10, pipa.position.x, pipa.position.y)
  console.log(wildPipa)
  setTimeout(() => {
    wildPipa.remove()
  }, 2000)
}
